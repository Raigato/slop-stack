...
pnpm env:load         # sync varlock env (regenerates env.d.ts)
```
...
**Environment variables:** Managed by [varlock](https://github.com/nickbreaton/varlock). `env.d.ts` is autogenerated — do not edit it directly. Run `pnpm env:load` after changing env schema.
